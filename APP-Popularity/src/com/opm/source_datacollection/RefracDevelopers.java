...
public class RefracDevelopers {
    ...
    private static void stats() throws Exception {
        ...
        for (int a = 0; a < files_merged1.length; a++) {
            int count = 0;
            int numbers = File_Details.getWorksheets(path_merged1 + files_merged1[a]);
            //System.out.println("Reading Collection Excel....!");
            while (count < numbers) {

                String project = File_Details.setProjectName(path_merged1 + files_merged1[a], count, "B2");
                List<String> names = ReadExcelFile_1Column.readColumnAsString(path_merged1 + files_merged1[a], count, 1, 1);
                List<String> changes1 = ReadExcelFile_1Column.readColumnAsString(path_merged1 + files_merged1[a], count, 4, 1);
                List<String> commits1 = ReadExcelFile_1Column.readColumnAsString(path_merged1 + files_merged1[a], count, 11, 1);
        ...
        for (int a = 0; a < files_merged2.length; a++) {
            int count = 0;
            int numbers = File_Details.getWorksheets(path_merged2 + files_merged2[a]);
            while (count < numbers) {

                ArrayList< Object[]> allobj = new ArrayList<Object[]>();
                List<String> dataList = new ArrayList<>();
                String project = File_Details.setProjectName(path_merged2 + files_merged2[a], count, "B2");
                List<String> n = ReadExcelFile_1Column.readColumnAsString(path_merged2 + files_merged2[a], count, 2, 2);
                List<String> refactor = ReadExcelFile_1Column.readColumnAsString(path_merged2 + files_merged2[a], count, 3, 1);
                List<String> changes = ReadExcelFile_1Column.readColumnAsString(path_merged2 + files_merged2[a], count, 4, 1);
                List<String> commits = ReadExcelFile_1Column.readColumnAsString(path_merged2 + files_merged2[a], count, 5, 1);
