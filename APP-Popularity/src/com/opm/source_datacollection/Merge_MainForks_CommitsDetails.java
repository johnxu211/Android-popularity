...
public class Merge_MainForks_CommitsDetails {
    ...
    private static void stats() throws Exception {
        ...
        String file_commits3 = "repos_commit_lg_mlp_fp101_150.xlsx";
        String file_commits4 = "repos_commit_lg_mlp_fp151_241.xlsx";
        ...
        for (int a = 0; a < files_commits.length; a++) {
            ...
            while (count < numbers) {
                ...
                String project = File_Details.setProjectName(path_commits + files_commits[a], count, "G2");
                String ml = File_Details.setProjectName(path_commits + files_commits[a], count, "G2");
                String fp = File_Details.setProjectName(path_commits + files_commits[a], count, "H2");
                
                List<String> date = ReadExcelFile_1Column.readColumnAsString(path_commits + files_commits[a], count, 0, 1);
                ...
                if (sheet.split("_").length > 2) {
                    project = File_Details.setProjectName(path_commits + files_commits[a], count, "H2");
                    //projNames = projNames.concat(project);
                }
                String subSheet = sheet.split("_")[0] + "_" + sheet.split("_")[1];
                System.out.println(count + " : " + s_c + " \t" + project + "\t" + date.size() + "\t" + commits.size());
                if (dateList.size() > 0) {
                    if (sheetL.contains(subSheet)) {
                        ...
                        if (count == numbers - 1) {
                            ...
                            for (int i = 0; i < dateList.size(); i++) {
                                ...
                                for (int j = 0; j < date_.size(); j++) {
                                    dList1.add(date_.get(j).split(" - ")[0]);
                                    dList2.add(date_.get(j).split(" - ")[1]);
                                    commits1.add(commits_.get(j));
                                }
                            }
                            String min_date = DateOperations.sorts(dList1, dList2).split("/")[0];
                            String max_date = DateOperations.sorts(dList1, dList2).split("/")[1];
                            ...
                            while (next < new_diff) {
                                ...
                                for (int i = 0; i < dList1.size(); i++) {
                                    if (Integer.parseInt(DateOperations.diff(min_date, dList1.get(i)).split("/")[0]) >= 0
                                            && Integer.parseInt(DateOperations.diff(dList1.get(i), next_date).split("/")[0]) >= 0) {
                            ...
                            Create_Excel.createExcel(allobj, 0, path_new + file_name, sheetN);
                        ...
                        Create_Excel.createExcel2(allobj, 0, path_new + file_name, sheetNN + "_" + sheet_c);
                        /// DOTO::::::::: ###################################
                        dateList = new ArrayList<>();
                        commitList = new ArrayList<>();
                        sheetL = new ArrayList<>();
