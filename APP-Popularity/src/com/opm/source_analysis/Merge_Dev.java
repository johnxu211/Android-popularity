...
public class Merge_Dev {
    ...
    private static void stats() throws Exception {
        ...
        for (int a = 0; a < files_google.length; a++) {
            ...
            while (count  < numbers) {
                //System.err.println("Number: "+(numbers-1));
                ...
                List<Double> commits = ReadExcelFile_1Column.readColumnAsNumeric(path_google + files_google[a], count, 3, 1);
                List<Double> unique = ReadExcelFile_1Column.readColumnAsNumeric(path_google + files_google[a], count, 4, 1);
                //List<String> commits_ = Pick_GeneralNext.pick(path_google + files_google[a], count, 6, 1);
                List<String> contribution = ReadExcelFile_1Column.readColumnAsString(path_google + files_google[a], count, 7, 1);
                List<String> percentage = ReadExcelFile_1Column.readColumnAsString(path_google + files_google[a], count, 8, 1);
                //List<String> category = Pick_GeneralNext.pick(path_google + files_google[a], count, 7, 1);
                    ...
                    for (int x = 0; x < lists_i.size(); x++) {
                        ...
                        for (int j = 0; j < lists_all.size(); j++) {
                            ...
                            for (int y = 0; y < lists_j.size(); y++) {
                                ...
                                /**
                                 * if (email_prefix_i.equals(email_prefix_j) &&
                                 * login_i.equals(login_j) &&
                                 * !login_i.equals("login######")) {// ||
                                 * name_i.equals(name_j) ||
                                 * email_prefix_i.equals(email_prefix_j)){ str1
                                 * = lists_i.get(x).substring(0,
                                 * lists_i.get(x).lastIndexOf(":")); str2 =
                                 * lists_j.get(y).substring(0,
                                 * lists_j.get(y).lastIndexOf(":")); if
                                 * (str1.length() > 5 && str2.length() > 5) {
                                 * String com_i =
                                 * lists_i.get(x).substring(lists_i.get(x).lastIndexOf(":")
                                 * + 1, lists_i.get(x).length()); String com_j =
                                 * lists_j.get(y).substring(lists_j.get(y).lastIndexOf(":")
                                 * + 1, lists_j.get(y).length());
                                 *
                                 * if (date_i.equals(date_j)) { double com = 0;
                                 * if (Coversions.isDouble(com_i) &&
                                 * Coversions.isDouble(com_j)) { com =
                                 * Double.parseDouble(com_i) +
                                 * Double.parseDouble(com_j); } //contrib_i =
                                 * contrib_i.replaceAll(split_i[x], str1 + ":" +
                                 * com); /// Delete from the list.. //contrib_i
                                 * = contrib_i.replaceAll(split_j[y], "");
                                 * //System.out.println(lists_i + "\t" +
                                 * split_i[x]); int index =
                                 * lists_i.indexOf(lists_i.get(x)); if (index !=
                                 * -1) { lists_i.set(index, str1 + ":" + com);
                                 * lists_i.remove(lists_i.indexOf(lists_j.get(y)));
                                 * lists_all.set(i, lists_i);
                                 * //lists_all.remove(lists_j); }
                                 * //System.out.println("1 : " +
                                 * contribution.get(i) + "\t" + split_i[x] +
                                 * "\t" + str1 + ":" + com);
                                 * //System.out.println("1... : " +
                                 * contribution.get(i) + "\t" + split_j[y]);
                                 *
                                 * } else { //contrib_j =
                                 * contrib_j.replaceAll(split_j[y], str1 +
                                 * com_j);
                                 * //System.out.println(lists_j+"\t"+split_j[y]);
                                 *
                                 * int index = lists_j.indexOf(lists_j.get(y));
                                 * if (index != -1) { lists_j.set(index, str1 +
                                 * ":" + com_j);
                                 *
                ...
                Create_Excel.createExcel(allobj, 0, path_new + file_name, project.split("/")[0] + "_" + count);
