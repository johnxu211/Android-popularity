...
import static com.opm.core.Coversions.isLong;
...
public class Merged_Repos2 {
    ...
    private static void doNameMerging() throws Exception {
        ...
        for (int aa = 0; aa < files.length; aa++) {
            ...
            int numbers = File_Details.getWorksheets(path + files[aa]);
            int count = 0;
            while (count < numbers) {
                double m_count = 0;
                ...
                allLists = Collections.readCommits(path + files[aa], count);
                ...
                String proj_name = File_Details.setProjectName(path + files[aa], count, "G2");
                String created_at = File_Details.setProjectName(path + files[aa], count, "G3");
                String proj_mastor = File_Details.setProjectName(path + files[aa], count, "G4");
...
//Create_Excel.createExcel(allobj, 0, path_new + f_name, File_Details.getWorksheetName(path + files[aa], count));
                    Create_Excel.createExcel(allobj2, 0, path_new + f_name2, proj_name.split("/")[0] + "_" + sheet_);
                    sheet_++;
                }
                count++;
            }// end of while loop...
