...
public class New_CommitsInterval {
     ...
     */
    public List<String> getCommitsNow(String projectName, String created_at, String proj_major, String date1, String date2, int i, String[] toks, String lastDate, double pl, double pc, double is, double ic, double ff, int ct, int com_index) throws org.json.simple.parser.ParseException, ParseException {
        ...
        while (true) {////loop thru the pagess....
            ...
            if (JSONUtilsConversion.isValidJSON(jsonString) == false) {///
                flag = 1;
            ...
            for (Object jsonObj : jsonArray) {
                ...
                if (JSONUtilsConversion.isValidJSONObject(commitsShaURL) == false) {///

                    System.out.println(shaa + " :Invalid Sha found!");
                    break;
                }
                if (JSONUtilsConversion.isValidJSONObject(commitsShaURL) == true) {///
            ...
            //      dList.add(del)
            // if(com_index == 0){
            //    System.out.println(projectName+" \t "+shaLists.get(shaLists.size()-1)+"\t"+cList.get(cList.size()-1)+"\t"+aList.get(cList.size()-1)+"\t"+dList.get(cList.size()-1)+"\t");
            // }
                ...
                for (int jj = 0; jj < lList.size(); jj++) {
                    if (lList.get(jj) != null && logList.get(ii) != null) {
                        if (lList.get(jj).equals(login_i)) {
                            if (ii < nameList.size() && ii < emailList.size()) {
